...
Performs type checking, scope analysis, and validates operations before interpretation.
Uses the traditional Visitor pattern (visit_ClassName methods).
"""
...
class NodeVisitor(object):
    """
    Base visitor class using the traditional Visitor pattern.
    Routes method calls to visit_ClassName methods based on node type.
    """
    def visit(self, node):
        # Dynamically construct method name: visit_ClassName
        method = 'visit_' + node.__class__.__name__
        # Get the method or fall back to generic_visit
...
class TypeChecker(NodeVisitor):
    """
    Type checker that validates the AST before interpretation.
    Checks:
    - Variable declarations and usage
    - Type compatibility in operations
    - Matrix/vector dimension compatibility
    - Break/continue statement placement
    - Index bounds
    """
    def __init__(self):
        # Symbol table for tracking variable types and scopes
        self.symbol_table = SymbolTable(None, "root")
        # Track nesting depth of loops (for break/continue validation)
        self.loop_depth = 0
        # List of error messages found during type checking
        self.errors = []
        
        # Type compatibility table for binary operations
        # Maps (operator, left_type, right_type) -> (result_type, constraint)
        # Constraints: None (no constraint), 'same_size' (operands must have same dimensions),
        #              'matrix_mul' (matrix multiplication dimension rules)
        self.ttype = {
    ...
    def error(self, msg, lineno=0):
        """
        Record and print a type checking error.
