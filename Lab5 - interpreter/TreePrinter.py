"""
Tree printer for visualizing the Abstract Syntax Tree.
Adds printTree() methods to AST node classes using a decorator pattern.
Used for debugging and understanding the structure of parsed programs.
"""
...
def addToClass(cls):
    ...
    Used to dynamically add printTree() methods to AST node classes.
    """
    def decorator(func):
...
class TreePrinter:
    """
    Tree printer that generates a text representation of the AST.
    Methods are dynamically added to AST node classes via the addToClass decorator.
    The output shows the tree structure with indentation.
    """
